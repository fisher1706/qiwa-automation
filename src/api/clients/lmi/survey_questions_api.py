...
class SurveyQuestionsApi:
    ...
    def put_question_weight(
        ...
        response = self.api.put(
            url=self.url, endpoint=f"/lmi-admin/surveys/{survey_id}/update-weight", json=json_body
        )
    ...
    def post_reset_weight(self, survey_id, expect_code=200):
        response = self.api.post(
            url=self.url, endpoint=f"/lmi-admin/surveys/{survey_id}/set-default-weights"
        )
        validator = ResponseValidator(response)
        validator.check_status_code(
    ...
    def put_question_code(self, survey_id, question_id, code, expect_code=200):
