...
from data.um.um_datasets import PaymentHeaders, SelfSubscriptionType
from data.um.um_users import owner_account
from src.api.app import QiwaApi
from src.api.models.qiwa.raw.um_models import SelfPriceCookie
...
class TestSelfSubscriptionAPI:  # pylint: disable=duplicate-code
    ...
    def test_self_extend_subscription_flow(self, subscr_type):
        ...
        self_subsc_cookie = SelfPriceCookie(user_id=user.user_id,
                                            company_sequence_number=user.sequence_number,
                                            company_labor_office_id=user.labor_office_id,
                                            user_personal_number=user.personal_number).dict(by_alias=True)
        self_price = float(qiwa.user_management_api.get_self_subscription_price(
            cookie=self_subsc_cookie,
            labor_office_id=user.labor_office_id,
            sequence_number=user.sequence_number
        ))
        transaction_id = int(qiwa.user_management_api.post_self_flow(
            cookie=self_subsc_cookie,
            labor_office_id=user.labor_office_id,
            sequence_number=user.sequence_number,
        ...
        qiwa.payment.post_confirm_payment(token=PaymentHeaders.Authorization, transaction_id=transaction_id)
